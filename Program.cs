...
var toolsList = new List<Tool> { new() { Name = "roslyn_ping", Description = "Проверка доступности сервера.", InputSchema = ToolSchemas.Ping() }, new() { Name = "roslyn_get_document_symbols", Description = "Структура C# файла: namespace, class, method, property, field с номерами строк.", InputSchema = ToolSchemas.GetDocumentSymbols() }, new() { Name = "roslyn_get_symbol_at_position", Description = "Символ в позиции (файл, строка, столбец — 1-based): kind и имя.", InputSchema = ToolSchemas.GetSymbolAtPosition() }, new() { Name = "roslyn_find_usages", Description = "Все ссылки на символ в solution/project. line/column — на идентификаторе (имя метода, класса и т.д.), не на типе в сигнатуре.", InputSchema = ToolSchemas.FindUsages() }, new() { Name = "roslyn_rename", Description = "Переименовать символ по solution. apply: false — превью; true — записать. Чтобы затронуть комментарии/строки/перегрузки/файл — передай rename_in_comments, rename_in_strings, rename_overloads, rename_file (bool).", InputSchema = ToolSchemas.Rename() }, new() { Name = "roslyn_get_code_actions", Description = "Список Quick Actions / рефакторингов в позиции (как лампочка в VS). Параметры: solution_or_project_path, file_path, line, column. Возвращает нумерованный список; применить — roslyn_apply_code_action с action_index.", InputSchema = ToolSchemas.GetCodeActions() }, new() { Name = "roslyn_apply_code_action", Description = "Применить выбранное code action по индексу из roslyn_get_code_actions. Параметры: solution_or_project_path, file_path, line, column, action_index (0-based).", InputSchema = ToolSchemas.ApplyCodeAction() }, new() { Name = "roslyn_get_diagnostics", Description = "Диагностики компиляции и анализаторов по solution/project. Предпочтительно использовать вместо разбора логов сборки. Чтобы исправить: вызвать roslyn_get_code_actions по file:line:column из ответа, затем roslyn_apply_code_action (или fix_all_scope). Параметры: solution_or_project_path, опционально file_path.", InputSchema = ToolSchemas.GetDiagnostics() }, };
